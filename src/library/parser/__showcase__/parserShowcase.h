...
void printHelpTest(ParsedOptions options) { ViewHelper::consoleOut("Hello, world!"); }
...
inline void runParser() {
    ...
    SchemeMap schemeMap = {
            {"start",
             ParserCommandInfo(
                     commandBuilder
                             ...
                             .addParameter(parameterBuilder.addFlag("--age")
                                                   .setValidator(std::regex("^[1-9][0-9]*?$"))
                                                   .setNecessary(true)
                                                   .buildAndReset())
                             .addParameter(parameterBuilder.addFlag("--name")
            ...
            {"end",
             ParserCommandInfo(
                     commandBuilder.setCallback(sayGoodbye)
                             .setDescription("The purpose of this function is to end an app")
                             .addParameter(parameterBuilder.addFlag("--hello")
                                                   .setValidator(std::regex("^[1-9][0-9]*?$"))
                                                   .buildAndReset())
                             .buildAndReset())},
    };
