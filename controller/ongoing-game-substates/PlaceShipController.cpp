    ...
    : context(context)
{
    ...
    gameFieldView = new GameFieldView(settings->getPlayerField()); // TODO: Fix GameFieldView;
...
bool PlaceShipController::allShipsPlaced() {
    return true;
    // return settings->getPlayerManager()->getShips().size() == currentPlayerManager->getShips().size();
}
